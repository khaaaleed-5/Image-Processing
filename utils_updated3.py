...
def histogram(image):
    ...
    """        
    gray_image = Gray_image(image) if image.ndim == 3 else image
...
#     """
#     Applies advanced edge detection (homogeneity) with padding
#     """
#     threshold_value = 5
#     kernel_size = 3  # Since the function uses 3x3 neighborhood
...
#     # Create an empty array to store the homogeneity image
#     homogeneity_image = np.zeros((height, width), dtype=np.float32)
   
#     # Iterate over the original image dimensions
#     for i in range(height):
#         for j in range(width):
#             # Extract the 3x3 region from padded image
#             region = padded_image[i:i+kernel_size, j:j+kernel_size]
            
#             center = region[pad, pad]
#             differences = [
#                 abs(float(center) - float(region[0, 0])),
#                 abs(float(center) - float(region[0, 1])),
#                 abs(float(center) - float(region[0, 2])),
#                 abs(float(center) - float(region[1, 0])),
#                 abs(float(center) - float(region[1, 2])),
#                 abs(float(center) - float(region[2, 0])),
#                 abs(float(center) - float(region[2, 1])),
#                 abs(float(center) - float(region[2, 2])),      
#             ]
...
def advanced_edge_rangeBased(image):
    """
    Applies range operator with padding.
    """
    gray_image = Gray_image(image)
